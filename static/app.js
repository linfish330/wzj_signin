(() => {
	...
	const HELP_TEXT =
		"快速开始\n" +
		"1. 在微助教学生页面完全加载前，点右上角三个点，选择“到浏览器打开”。\n" +
		"2. 从新链接中复制 openid 字段的 32 位值；先在设置页保存自己的邮箱。\n" +
		"3. 在提交页粘贴 OpenID/链接，按需选择 GPS 标签并提交。\n" +
		"4. 所有 OpenID 最长只有 2 小时有效。提交后不要再打开任何微助教页面，否则可能提前失效。\n" +
		"5. 普通签到和 GPS 签到成功加入监控后无需操作，签到成功会收到邮件提醒。\n" +
		"6. 二维码签到会立即邮件发送带链接的实时二维码，用微信扫一扫；扫码后原 OpenID 立即失效。\n" +
		"7. 若要再次签到或多次签到，请重新完成以上获取 OpenID 的流程。\n\n" +
		"注意\n" +
	...
	function setHelpText() {
		...
		if (noticeBox) {
			noticeBox.innerHTML =
				"1）微助教学生页面完全加载前，点右上角三个点，选择“到浏览器打开”，复制链接中的 32 位 OpenID<br />" +
				"2）到【设置】保存自己的邮箱，并按需添加 GPS 标签（可选）<br />" +
				"3）到【提交】粘贴 OpenID/链接，选择 GPS 标签并提交；OpenID 最长有效 2 小时<br />" +
				"4）提交后不要再打开任何微助教页面，否则 OpenID 可能提前失效<br />" +
				"5）普通/GPS 签到成功加入监控后无需操作，成功会收到邮件提醒<br />" +
				"6）二维码签到会立即邮件发送实时二维码链接，用微信扫一扫；扫码后原 OpenID 立即失效，再次签到需重新获取<br /><br />" +
